...
passport.serializeUser((user, done) => {
  ...
  // req.session.cookie = {user_id: '簽名後的id'}
  // req.isAuthenticated()會回傳true
});
  ...
  new GoogleStrategy(
    ...
    async (accessToken, refreshToken, profile, done) => {
      ...
      if (foundUser) {
        ...
        let savedUser = await newUser.save(); //save() return a promise 
        console.log("成功創建新用戶。");
        done(null, savedUser); // 會觸發serializeUser
      }
    }
  )
);
